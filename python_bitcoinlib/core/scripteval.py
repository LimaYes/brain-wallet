...
import python_bitcoinlib.core._bignum
import python_bitcoinlib.core.key
import python_bitcoinlib.core.serialize
...
from python_bitcoinlib.core.script import *
...
class EvalScriptError(python_bitcoinlib.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = python_bitcoinlib.core._bignum.vch2bn(s)
...
def _CheckSig(sig, pubkey, script, txTo, inIdx, err_raiser):
    key = python_bitcoinlib.core.key.CECKey()
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(python_bitcoinlib.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(python_bitcoinlib.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(python_bitcoinlib.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(python_bitcoinlib.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(python_bitcoinlib.core.ValidationError):
    pass
...
class VerifySignatureError(python_bitcoinlib.core.ValidationError):
    pass
