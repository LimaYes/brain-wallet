...
import python_bitcoinlib.base58
import python_bitcoinlib.core
import python_bitcoinlib.core.key
import python_bitcoinlib.core.script as script

class CBitcoinAddressError(python_bitcoinlib.base58.Base58Error):
    """Raised when an invalid Bitcoin address is encountered"""

class CBitcoinAddress(python_bitcoinlib.base58.CBase58Data):
    """A Bitcoin address"""
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == python_bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == python_bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']:
...
class P2SHBitcoinAddress(CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = python_bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != python_bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(scriptPubKey[2:22], python_bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == python_bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']
...
class P2PKHBitcoinAddress(CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = python_bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        ...
        if not accept_invalid:
            if not isinstance(pubkey, python_bitcoinlib.core.key.CPubKey):
                pubkey = python_bitcoinlib.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = python_bitcoinlib.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except python_bitcoinlib.core.script.CScriptInvalidError:
        ...
        if (len(scriptPubKey) == 25
            ...
            return cls.from_bytes(scriptPubKey[3:23], python_bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == python_bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = python_bitcoinlib.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = python_bitcoinlib.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(python_bitcoinlib.base58.Base58Error):
    pass

class CBitcoinSecret(python_bitcoinlib.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              python_bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != python_bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d' % \
                                      (self.nVersion, python_bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']))
