...
class CBloomFilter(python_bitcoinlib.core.serialize.Serializable):
    ...
    def insert(self, elem):
        ...
        """
        if isinstance(elem, python_bitcoinlib.core.COutPoint):
            elem = elem.serialize()
    ...
    def stream_deserialize(cls, f):
        vData = bytearray(python_bitcoinlib.core.serialize.BytesSerializer.stream_deserialize(f))
        (nHashFuncs,
         nTweak,
         nFlags) = CBloomFilter.__struct.unpack(python_bitcoinlib.core.ser_read(f, CBloomFilter.__struct.size))
    ...
    def stream_serialize(self, f):
        if sys.version > '3':
            python_bitcoinlib.core.serialize.BytesSerializer.stream_serialize(self.vData, f)
        else:
            # 2.7 has problems with f.write(bytearray())
            python_bitcoinlib.core.serialize.BytesSerializer.stream_serialize(bytes(self.vData), f)
