...
# later.
from python_bitcoinlib.core import *
from python_bitcoinlib.core.serialize import *
from python_bitcoinlib.net import *
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = python_bitcoinlib.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != python_bitcoinlib.params.MESSAGE_START:
            raise ValueError("Invalid message start '%s', expected '%s'" %
                             (b2x(recvbuf[:4]), b2x(python_bitcoinlib.params.MESSAGE_START)))
...
class msg_version(MsgSerializable):
    ...
    def __init__(self, protover=PROTO_VERSION):
        ...
        self.strSubVer = (b'/python-bitcoinlib:' +
                          python_bitcoinlib.__version__.encode('ascii') + b'/')
